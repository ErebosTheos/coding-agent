...
from .models import PipelineReport, TestSuite
...
class Orchestrator:
    ...
    async def run(self, prompt: str, resume: bool = False) -> PipelineReport:
            ...
            if not report.dependency_resolution:
                ...
                pending["dep"] = asyncio.create_task(
                    dep_manager.resolve_and_install(
                        report.execution_result.generated_files, report.plan
                    )
                )
        ...
        if not report.healing_report:
            ...
            healer = Healer(self.router.get_client_for_role("healer"), self.workspace)
            consistency_issues = _collect_python_consistency_issues(report.execution_result.generated_files)
            static_attempts = []
            if consistency_issues:
                print(
                    "  [Orchestrator] Found static consistency issues in generated source. "
                    "Applying targeted source fixes before test healing."
                )
                static_attempts = await healer.heal_static_issues(consistency_issues, attempt_number=0)
        ...
        if not report.qa_report:
            ...
            qa_task = asyncio.create_task(
                QAAuditor(self.router.get_client_for_role("qa_auditor")).audit(report)
            )
        ...
        report = replace(report, wall_clock_seconds=time.monotonic() - start_time)
